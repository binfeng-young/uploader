...
void ssp::ssp_Init(const PortConfig_t *const info)
{
 ...
 *
 */
int16_t ssp::ssp_ReceiveProcess()
{
    int16_t b;
 ...
 *
 */
uint16_t ssp::ssp_SendDataBlock(uint8_t *data, uint16_t length)
{
...
void ssp::sf_SendAckPacket(uint8_t seqNumber)
{
 ...
 *
 */
void ssp::sf_write_byte(uint8_t c)
{
...
uint16_t ssp::sf_crc16(uint16_t crc, uint8_t data)
{
#ifdef SPP_USES_CRC
 ...
 *
 */
int16_t ssp::sf_ReceiveState(uint8_t c)
{
 ...
 *
 */
int16_t ssp::sf_DecodeState(uint8_t c)
{
    int16_t retval;
...
ssp::ssp(port *info, bool debug) : debug(debug)
{
    thisport = info;
    thisport->maxRetryCount = info->max_retry;
    thisport->timeoutLen    = info->timeoutLen;
...
void ssp::pfCallBack(uint8_t *buf, uint16_t size)
{
//    Q_UNUSED(size);
//
//    if (debug) {
